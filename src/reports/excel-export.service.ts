...
export class ExcelExportService {
  ...
  private async createPaymentsSheet(sheet: ExcelJS.Worksheet, startDate?: Date, endDate?: Date) {
    ...
    payments.forEach((payment, index) => {
      const row = sheet.addRow({
        ...
        customer: payment.loan && payment.loan.customer ? `${payment.loan.customer.firstName} ${payment.loan.customer.lastName}`.trim() : 'N/A',
      ...
      ['totalAmount', 'interestPaid', 'capitalPaid', 'remainingBalance'].forEach(col => {
  ...
  private async createSummarySheet(sheet: ExcelJS.Worksheet, startDate?: Date, endDate?: Date) {
    sheet.columns = [
    ...
    const summaryData = [
      { concept: 'Período del Reporte', value: this.formatDateRange(startDate, endDate) },
    ...
    summaryData.forEach((item, index) => {
      ...
      if (item.concept.includes('MÉTRICAS') || item.concept.includes('ESTADO') || item.concept.includes('RENDIMIENTO')) {
        row.font = { bold: true };
        row.fill = {
          type: 'pattern',
          pattern: 'solid',
  ...
  private async calculateStats(startDate?: Date, endDate?: Date) {
    ...
    const totalAmount = payments.reduce((sum, p) => new Decimal(sum).plus(new Decimal(p.amount)), new Decimal(0)).toNumber();
    const totalInterest = payments.reduce((sum, p) => new Decimal(sum).plus(new Decimal(p.interestPaid || 0)), new Decimal(0)).toNumber();
    const totalCapital = payments.reduce((sum, p) => new Decimal(sum).plus(new Decimal(p.capitalPaid || 0)), new Decimal(0)).toNumber();
    ...
    const capitalInTransit = allLoans
      ...
      .reduce((sum, l) => new Decimal(sum).plus(new Decimal(l.currentBalance || 0)), new Decimal(0)).toNumber();
    ...
    return {
      ...
      overdueLoans,
      capitalInTransit,
      averagePayment: totalPayments > 0 ? new Decimal(totalAmount).dividedBy(totalPayments).toNumber() : 0,
      monthlyInterestRate: totalAmount > 0 ? new Decimal(totalInterest).dividedBy(totalAmount).times(100).toNumber() : 0,
    };
  }
